...
def ppa2d(group, title, v_range=None, w_range=None, Iext=65., duration=400):
  ...
  phase_plane_analyzer = bp.analysis.PhasePlane2D(
    model=group,
    target_vars={'w': w_range, 'V': v_range, },  # 待分析变量
  ...
  for i in range(len(s_idx) - 1):
    plt.plot(runner.mon.w[s_idx[i]: s_idx[i + 1]],
             runner.mon.V[s_idx[i]: s_idx[i + 1]],
             color='darkslateblue')
  # 画出虚线 x = V_reset
