...
X, Y = data[:-5], data[5:]  # Y比X滞后5个步长，即需要预测系统未来的Y
# 将第0维扩展为batch的维度
...
def training_lorenze(trainer, title):
  ...
  plt.plot(Y[0, :, 0], Y[0, :, 1], Y[0, :, 2], alpha=0.8, label='standard output')
  plt.plot(predict[0, :, 0], predict[0, :, 1], predict[0, :, 2], alpha=0.8, label='prediction')
  plt.title(title)
  plt.legend()

  fig.add_subplot(222)
  t = np.arange(Y.shape[1])
  plt.plot(t, Y[0, :, 0], linewidth=1, label='standard $x$')  # 劳伦兹系统中的x变量
  plt.plot(t, predict[0, :, 0], linewidth=1, label='predicted $x$')
  plt.ylabel('x')

  fig.add_subplot(224)
  plt.plot(t, Y[0, :, 2], linewidth=1,label='standard $z$')  # 劳伦兹系统中的z变量
  plt.plot(t, predict[0, :, 2], linewidth=1, label='predicted $z$')
  plt.ylabel('z')
